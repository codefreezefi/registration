...
export const Submit = () => {
	...
	const needsNoInvitationLetter = () =>
		(registration.needsInvitationLetter ?? false) === false
	const isValid = () =>
		[
			isValidName(),
			isValidEmail(),
			isCodeOfConductAccepted(),
			needsNoInvitationLetter(),
		].reduce((allValid, v) => (v === false ? false : allValid), true)
					...
					fallback={
						<>
							...
							<ol>
								...
								<Show when={!needsNoInvitationLetter()}>
									<li>
										<a href="#invitation-letter-form">
											Please acknowledge that we will not provide an invitation
